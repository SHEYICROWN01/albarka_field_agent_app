...
class _ConfirmUpdateProfileState extends State<ConfirmUpdateProfile> {
  ...
  Future updateMember() async {
    try {
      ...
      final url = Uri.parse(
          'https://dashboard.albarkaltd.com/mobileapi/updateMember.php');
  ...
  Future updateMemberProfilePicture() async {
    try {
      ...
      final url = Uri.parse(
          'https://dashboard.albarkaltd.com/mobileapi/updateMemberPicture.php');
      var request = http.MultipartRequest('POST', url);
      request.fields['member_id'] = customerId!.text;
      var pic = await http.MultipartFile.fromPath("image", _image!.path);
      request.files.add(pic);
                    ...
                    child: CircleAvatar(
                      ...
                      child: CircleAvatar(
                          ...
                          backgroundImage: NetworkImage(
                              'https://dashboard.albarkaltd.com/${widget.list[widget.index].picture}')),
                    ),
                  ),
                )
              ],
